...
> Speaker: {self.speaker}
    ...
    def printPrep(self, offset, alt):
        ...
        #use sentence with TC highlighted
        elif alt == 1:
            if self.tensedClause == "X":
                sentence = self.sentence.split()
                index = [item for item in range(0, len(sentence)+offset)]
                index = ["\033[32m" + str(item) + "\033[0m" for item in index]
                self.printFormat(index, sentence)
                ...
                sentence = self.sentence.split() #list
                
                tClause = self.tensedClause.split() #list
                lowlight = ["\033[34m" + word + "\033[0m" for word in sentence] #list
                tClauseLowlight = ["\033[034m" + word + "\033[0m" for word in tClause] #list
                sentenceJoined = " ".join(lowlight)  #string
                sentenceJoined.replace(" ".join(tClauseLowlight), self.tensedClause)
                final = sentenceJoined.split()
                index = [item for item in range(0, len(final)+offset)]
                index = ["\033[32m" + str(item) + "\033[0m" for item in index]
                self.printFormat(index, final)
    ...
    def markVerb(self):
        ...
        while True:
            try:
                ...
                if verbInput == "":
                    print("\n \033[31m!Invalid input\n")
                elif verbInput in ("MV", "mv"):
                    mvIndex = input("Input the index at which the missing verb should appear: ")
                    self.insertVerb(mvIndex)
                    return verbInput
                else:
                    return verbInput
            except ValueError:
    ...
    def insertVerb(self, index):
        outSentence = self.tensedClause.split()
        preVerb = outSentence[0:index]
        postVerb = outSentence[index:]
        verbInserted = preVerb + "MV" + postVerb
        self.tensedClause = verbInserted
...
def splashScreen(inputFileName, outputFileName):
    ...
    print(f"""\033[0m
                                    ...
                                    Type CTRL+C to exit the program at any time.
        ...
        while True:
            try:
                ...
                with open(sys.argv[2], "a+") as outputFile:
                    numLines, codedBy = splashScreen(sys.argv[1], sys.argv[2])
                ...
                while True:
                    answer = str(
                        input('  \033[0mWould you like to keep coding? (y/n):\033[36m '))
                    if answer in ('y', 'n'):
                        break
